...
class Exercise(models.Model):
    ...
    example_audio_url = models.CharField(
        max_length=500, blank=True, default='', help_text='Path to example audio file'
    )
    order = models.PositiveIntegerField(default=0, help_text='Display order')
    is_active = models.BooleanField(
        default=True, help_text='Whether this exercise is currently in use'
    )
...
class Patient(models.Model):
    ...
    # Operation dates
    pre_op_date = models.DateTimeField(null=True, blank=True, verbose_name='Prä-OP Datum')
    post_op_date = models.DateTimeField(null=True, blank=True, verbose_name='Post-OP Datum')

    # --- Pre-Op AI Results ---
    prediction_pre = models.CharField(
        max_length=10,
        choices=PredictionStatus.choices,
        default=PredictionStatus.TODO,
        verbose_name='KI-Vorhersage (Prä-OP)',
    )
    ai_percentage_rp_pre = models.FloatField(
        null=True, blank=True, verbose_name='KI Wahrscheinlichkeit Prä-OP (%)'
    )
    gradcam_prediction_pre = models.CharField(
        max_length=20, blank=True, default='', verbose_name='GradCAM Vorhersage (Prä-OP)'
    )
    gradcam_percentage_pre = models.FloatField(
        null=True, blank=True, verbose_name='GradCAM Wahrscheinlichkeit Prä-OP (%)'
    )
    ai_reasoning_pre = models.TextField(
        blank=True, default='', verbose_name='KI-Begründung (Prä-OP)'
    )
    ...
    ai_percentage_rp_post = models.FloatField(
        null=True, blank=True, verbose_name='KI Wahrscheinlichkeit Post-OP (%)'
    )
    gradcam_prediction_post = models.CharField(
        max_length=20, blank=True, default='', verbose_name='GradCAM Vorhersage (Post-OP)'
    )
    gradcam_percentage_post = models.FloatField(
        null=True, blank=True, verbose_name='GradCAM Wahrscheinlichkeit Post-OP (%)'
    )
    ai_reasoning_post = models.TextField(
        blank=True, default='', verbose_name='KI-Begründung (Post-OP)'
    )
    ...
    expires_at = models.DateTimeField(
        verbose_name='Läuft ab am', help_text='Datum, ab dem der Datensatz gelöscht werden soll'
    )
    ...
    def last_activity(self):
        ...
        if (
            ...
            and 'audio_files' in self._prefetched_objects_cache
        ):
        ...
        if (
            hasattr(self, '_prefetched_objects_cache')
            and 'feedback_entries' in self._prefetched_objects_cache
        ):
        ...
        if (
            hasattr(self, '_prefetched_objects_cache')
            and 'exercise_skips' in self._prefetched_objects_cache
        ):
...
class RecordingSession(models.Model):
    ...
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name='sessions', verbose_name='Patient'
    )
    phase = models.CharField(max_length=10, choices=Phase.choices, verbose_name='Phase')
    ...
    def __str__(self):
        return (
            f'{self.get_phase_display()} Sitzung {self.session_number} ({self.patient.patient_id})'
        )
...
class AudioFile(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name='audio_files', verbose_name='Patient'
    )
    ...
    exercise_id = models.CharField(
        max_length=50, verbose_name='Übungs-ID', help_text='References Exercise.exercise_id'
    )
    phase = models.CharField(
        max_length=10, choices=Phase.choices, default=Phase.PRE_OP, verbose_name='Phase'
    )
...
class PatientFeedback(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name='feedback_entries', verbose_name='Patient'
    )
    phase = models.CharField(
        max_length=10, choices=RecordingSession.Phase.choices, verbose_name='Phase'
    )
...
class ExerciseSkip(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name='exercise_skips', verbose_name='Patient'
    )
    session = models.ForeignKey(
        RecordingSession,
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name='exercise_skips',
        verbose_name='Sitzung',
    )
    phase = models.CharField(
        max_length=10,
        choices=RecordingSession.Phase.choices,
        verbose_name='Phase',
